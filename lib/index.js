...
function formatcheck(config) {
  ...
  return function(files, metalsmith, done) {
    ...
    var htmlfiles = _.pick(files, function(file, filename) {
      return (path.extname(filename) === '.html' && !(file.checkFormat === false));
    });
    ...
    async.series([
        function (callback) {
          request("http://www.google.com", function (error, response, body) {
            if (error || !response || response.statusCode != 200) {
              console.log("metalsmith-formatcheck: network failure");
        ...
        function (callback) {
          async.forEachOfLimit(htmlfiles, config.concurrency, function (file, filename, finished) {
            ...
            validator({ validator: config.validator, format: 'text', data: file.contents}, function (err, data) {
              if (err) {
